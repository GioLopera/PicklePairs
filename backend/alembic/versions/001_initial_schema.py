...
def upgrade() -> None:
    ...
    if "rooms" not in existing:
        op.create_table(
            ...
            sa.Column(
                "status",
                postgresql.ENUM("open", "closed", name="room_status", create_type=False),
                nullable=False,
                server_default="open",
            ),
    ...
    if "team_results" not in existing:
        op.create_table(
            "team_results",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
            sa.Column("room_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("rooms.id"), nullable=False),
    ...
    if "team_assignments" not in existing:
        op.create_table(
            "team_assignments",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
            ...
            sa.Column(
                "status",
                postgresql.ENUM("playing", "waiting", name="assignment_status", create_type=False),
                nullable=False,
